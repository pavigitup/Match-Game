...
class App extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      selectedImageId: initialImagesList.find(item => item.category === 'FRUIT')
        .id,
      score: 0,
      gameOver: false,
    }

    // Bind the startGame method to the instance
    this.resetBtn = this.resetBtn.bind(this)
    this.startTimer = this.startTimer.bind(this)
    this.endGame = this.endGame.bind(this)
    this.filteredLists = this.filteredLists.bind(this)
    this.getRandomImage = this.getRandomImage.bind(this)
    this.getTabList = this.getTabList.bind(this)
    this.resetBtn = this.resetBtn.bind(this)
    this.fruitId = this.fruitId.bind(this)
  }
  ...
  startTimer = () => {
    this.timerIntervalId = setInterval(() => {
      ...
      if (timerLimit > 0) {
        ...
        const currentTime = timerLimit
        this.endGame()
        this.setState({timerLimit: currentTime})
      }
    }, 1000)
  }
  ...
  fruitId = id => {
    const {selectedImageId, score, timerLimit} = this.state

    console.log(id)

    const selectedImage = initialImagesList.find(
      eachTab => eachTab.id === selectedImageId,
    )
    ...
    if (selectedImage.id === id) {
      ...
      this.setState({
        selectedThumbnailUrl: randomImage.imageUrl,
  ...
  render() {
    const {timerLimit, selectedThumbnailUrl, score, gameOver} = this.state
        ...
        {gameOver ? (
          <div className="game-over-con">
            <div className="game">
              ...
              />
              <p>Your Score</p>
              <p className="score-size">{score}</p>
              <button
                type="button"
          ...
          <div className="game-con">
            <ul>
              <NavItems imageUrl={selectedThumbnailUrl} />
            </ul>
